...
class BanditAutoscaler(object):
    ...
    def run(self, num_iters=100000, wait_time=15):
    ...
    def query_model2(self):
        ...
        # Get the upper bound from our context.
        query_rps_ub = None
        for context in sorted(self.train_context_rps2):
            if context > self.current_rps:
                query_rps_ub = context
                break
        ...
        # Handle rps > all trained context.
        if query_rps_ub is None:
            query_rps_ub = self.train_context_rps2[-1]
            query_rps_lb = self.train_context_rps2[-1]

        # Handle rps == 0.
        elif self.current_rps == 0:
            query_rps_ub = self.train_context_rps2[0]
            query_rps_lb = self.train_context_rps2[0]

        # Handle rps is in our training range.
        else:
            query_rps_lb = self.train_context_rps2[self.train_context_rps2.index(query_rps_ub)-1]
        ...
        # Convert RPS to input users we specified.
        config_lb = self.train_config2.config_map[self.train_context2[self.train_context_rps2.index(query_rps_lb)]]
        config_ub = self.train_config2.config_map[self.train_context2[self.train_context_rps2.index(query_rps_ub)]]
        
        d_dist = np.linalg.norm(self.req_vector, self.context_map[w_ub]['req_vector'])

        return config_lb, config_ub, w_lb, w_ub, d_dist # Return interpolation info for best model given current rps.
    ...
    def get_pod_config2(self):
        ...
        # Get model for current rps.
        model_lb, model_ub, w_lb, w_ub = self.query_model2()
        ...
        logger.info("Current policy: {}".format(scale_config))
        return scale_config, d_dist
        

    def get_pod_config_weighted(self, conf1, conf2, w1, w2):

        weighted_config = {}
        for service in conf1:
            
            num_replicas = w1*conf1[service] + w2*conf1[service]
            num_replicas = int(math.ceil(num_replicas))
            num_replicas = max(num_replicas, min_replicas)
            num_replicas = min(num_replicas, max_replicas)

            weighted_config[service] = num_replicas
        
        logger.info("Current policy: {}".format(weighted_config))
